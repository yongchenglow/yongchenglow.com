...
```bash
helm upgrade --install web ./helm/app \
...
```bash
helm status web --namespace personal-site
...
```bash
kubectl port-forward service/web 8080:3000 \
  --namespace personal-site
```
...
```bash
helm history web --namespace personal-site
REVISION=2
helm rollback web "$REVISION" --namespace personal-site --wait --timeout 10m
```

Remove the release when it is no longer needed.

```bash
helm uninstall web --namespace personal-site
```
...
kubectl describe deployment/web --namespace personal-site
kubectl describe pod --selector app.kubernetes.io/name=web \
  --namespace personal-site
kubectl logs --selector app.kubernetes.io/name=web \
  --namespace personal-site \
  --all-containers
```
