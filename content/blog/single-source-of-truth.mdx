...
**[Database constraints](https://www.postgresql.org/docs/current/ddl-constraints.html)** give you control over the data stored inside the tables
...
The `numeric(5,2)` allows us to store numbers with up to 5 total digits, of which 2 are decimal places (e.g. -999.99 to 999.99)
...
It is important to note that a unique constraint can also be placed on multiple columns at the same time. For example, if we want to go back to our trustworthy AirBnB application, lets say we want to create a favorites table to store flats that the user saved. In order to prevent double insertions (saving to favorites twice), we add a unique constraint on user_id and flat_id
