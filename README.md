...
1. bun install
2. bun run prepare (Only for development setup)
3. bun run dev
...
```bash
bun run dev                    # Start development server (generates search index first)
bun run build                  # Build for production (generates search index first)
bun start                      # Run production build
bun run generate-search-index  # Generate search index from blog posts
bun run lqip                   # Generate LQIP images for blog post images
...
bun run check:ci               # Run Biome checks for CI
bun test                       # Run tests with the Bun test runner
bun run typecheck              # Run TypeScript type checking
bun run tsc                    # Alias for typecheck
...
test/
├── setup.ts                    # Bun test preload / setup file
...
4. Run `bun run dev` — the search index regenerates automatically
...
- **Triggers:** Runs automatically before `bun run dev` and `bun run build`
...
Tests are run using the Bun test runner with a happy-dom environment and React Testing Library.
