...
The browser downloads that JSON the first time search is opened. `src/hooks/useSearch.ts` builds a FlexSearch document index for the title, subtitle, description, content, and tags. It merges duplicate posts found across those fields before displaying the results.
...
GitHub Actions handles checks, container publishing, security scanning, production deployment, and pull request review environments. Kubernetes settings live in `helm/app/`.

See the [Helm chart guide](helm/app/README.md) for local chart validation and manual deployment commands.
